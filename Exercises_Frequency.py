...
def make_display_magnitude(mag):
    ...
    cv2.normalize(display_mag, display_mag, alpha=0, beta=1, norm_type=cv2.NORM_MINMAX)
    return display_mag
...
def complex_image_to_display_mag(complex_image):
    complex_data = to_numpy_complex(complex_data)
...
def filter_with_fourier(image, kernel):
    ...
    padded_cols = cv2.getOptimalDFTSize(image.shape[1] + kernel.shape[1])
    
    fimage = image.astype("float64")
    fkernel = kernel.astype("float64")
    
    padded_image = cv2.copyMakeBorder(fimage, 0, padded_rows-image.shape[0],
                                      0, padded_cols - image.shape[1],
                                      borderType=cv2.BORDER_CONSTANT,
                                      value = 0)

    padded_kernel = cv2.copyMakeBorder(fkernel, 0, padded_rows-kernel.shape[0],
    ...
    nG = to_numpy_complex(G)
    mag, phase = complex_to_polar(nG)
    display_mag = make_display_magnitude(mag)
    ...
    padded_output = compute_inverse_fourier(G, nonzeroRows = image.shape[0] + kernel.shape[0] - 1)
