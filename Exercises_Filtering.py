...
class FilterType(Enum):
    ...
    SHARP_LAPLACE = "Laplacian Filter"
    SOBEL_X = "Sobel X"
    SOBEL_Y = "Sobel Y"
...
def do_filter(image, filter_size, filter_type):
    if filter_type == FilterType.BOX:
        ...
        laplace = cv2.Laplacian(image, ddepth=cv2.CV_64F, ksize = filter_size, scale = 0.25)
        ...
        output = cv2.convertScaleAbs(sx, alpha=0.5, bet=127)
    elif filter_type == FilterType.SOBEL_Y:
        sy = cv2.Sobel(image, cv2.CV_64F, dx=0, dy=1, ksize=3, scale=0.25)
        output = cv2.convertScaleAbs(sy, alpha=0.5, bet=127)
...
def main():        
    ...
    # image = np.array([[0,1,2,3],[3,2,1,0], [2,0,3,1]], dtype="uint8")
    # print(image)
    # print(image.shape)
    ...
    while key != ESC_KEY:
        ...
        sobelX = cv2.Sobel(grayscale, cv2.CV_64F, dx=1, dy=0, ksize=3, scale=1.0/(4.0*255))
